// Audio Monitor - Cliente Web con WebSocket y WebRTC
class AudioMonitor {
    constructor() {
        this.socket = null;
    ...
    async init() {
        ...
        // Conectar WebSocket (para señalización WebRTC)
    ...
    checkWebRTCAvailability() {
        if (!window.RTCPeerConnection || !window.RTCSessionDescription) {
            console.warn('[WebRTC] No disponible en este navegador');
            this.useWebRTC = false;
            this.protocol = 'websocket';
            this.metrics.protocol = 'WebSocket';
            this.showWarning('WebRTC no disponible. Usando WebSocket.');
        } else {
            console.log('[WebRTC] Disponible');
        }
    }
    ...
    async connectWebSocket() {
        ...
        this.socket.on('connect', () => {
            ...
            // Iniciar WebRTC si está disponible
            if (this.useWebRTC) {
                this.initWebRTC();
            } else {
        ...
        // WebSocket audio (solo si usamos WebSocket)
        ...
        this.socket.on('webrtc_answer', (data) => {
            if (this.webrtcClient) {
                this.webrtcClient.handleAnswer(data.sdp);
            }
        });
        
        this.socket.on('webrtc_error', (data) => {
            console.error('[WebRTC] Error:', data.error);
            this.fallbackToWebSocket(`WebRTC error: ${data.error}`);
        });
        
        this.socket.on('webrtc_subscribed', (data) => {
            console.log('[WebRTC] Suscrito a canales:', data.channels);
        });
    ...
    handleDeviceInfo(info) {
        ...
        // WebRTC ya se inicia automáticamente después de connect
    }
    ...
    async initWebRTC() {
        ...
        try {
            ...
            this.webrtcClient.onError = (error) => {
                ...
                this.fallbackToWebSocket(`WebRTC error: ${error.message || 'Error desconocido'}`);
            };
    ...
    displayDeviceInfo(info) {
        ...
        if (el) {
            el.innerHTML = `
                ...
                Protocolo: ${this.metrics.protocol}
            `;
        }
    }
    ...
    async toggleChannel(channel, button) {
        ...
        console.log(`[App] Canales activos después del toggle: ${this.activeChannels.size}`);
        console.log(`[App] Llamando a updateSubscription()...`);
        
        // ✅ CRÍTICO: Llamar a updateSubscription
    ...
    updateSubscription() {
        ...
        if (this.protocol === 'websocket' && this.socket?.connected) {
            ...
            this.webrtcClient.subscribe(channels, gains);
            console.log('[WebRTC] Suscripción actualizada:', channels);
        } else if (this.protocol === 'webrtc') {
            console.log('[WebRTC] No conectado aún, esperando...');
            // Reintentar en 500ms
            setTimeout(() => this.updateSubscription(), 500);
        }
    }
    ...
    updateProtocolDisplay(protocol) {
        ...
        if (el) {
            el.textContent = protocol || this.metrics.protocol;
