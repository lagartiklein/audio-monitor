...
class NativeAudioClient {
    ...
    companion object {
        ...
        private const val FLAG_COMPRESSED = 0x04 // ✅ NUEVO: Compresión Zlib
    ...
    private fun startReaderThread() {
        CoroutineScope(Dispatchers.IO).launch {
            // ✅ Configurar prioridad de audio
            try {
                Process.setThreadPriority(Process.THREAD_PRIORITY_URGENT_AUDIO)
            } catch (e: Exception) {
                Log.w(TAG, "⚠️ Prioridad URGENT_AUDIO no soportada")
            }
        ...
        try {
            ...
            when (msgType) {
                ...
                "master_gain_update" -> {
                    ...
                    Log.d(TAG, "📥 master_gain_update: ${gainDb}dB")
                ...
                "mix_state" -> {
                    ...
                    Log.d(TAG, "📥 mix_state: ${channels.size} canales, ${gains.size} gains")
        ...
        try {
            ...
            val interleavedAudioData = if (isCompressed) {
                ...
                try {
                    ...
                    Log.e(TAG, "❌ Error descomprimiendo: ${e.message}")
                    return null
                }
            } else {
                // Audio sin comprimir
