        ...
        audioClient = NativeAudioClient(this).apply {
            ...
            onChannelUpdate = { channel, gainDb, pan, active ->
                lifecycleScope.launch(Dispatchers.Main) {
                    if (!isFinishing && !isDestroyed) {
                        gainDb?.let { audioRenderer.updateChannelGain(channel, it) }
                        pan?.let { audioRenderer.updateChannelPan(channel, it) }
                        active?.let { audioRenderer.setChannelActive(channel, it) }
                    }
                }
            }
